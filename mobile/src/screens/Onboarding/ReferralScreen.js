...
export default function ReferralScreen({ navigation }) {
  ...
  return (
    <View style={{ flex: 1, backgroundColor: colors.bg, padding: 24, justifyContent: 'center' }}>
      <Text style={{ color: colors.text, fontSize: 24, fontWeight: '700', marginBottom: 12 }}>Enter your referral code</Text>
      <TextInput value={code} onChangeText={(t) => setCode(t.replace(/\D/g, '').slice(0, 4))} keyboardType="number-pad" placeholder="0000" placeholderTextColor="#aaa" style={{ color: colors.text, borderColor: colors.text, borderWidth: 1, borderRadius: 12, padding: 16, letterSpacing: 8, textAlign: 'center', fontSize: 20 }} />
      <Text style={{ color: colors.textMuted, marginTop: 8 }}>Don't have referral code?</Text>
      ...
      <TouchableOpacity onPress={() => handleContinue(true)} style={{ borderRadius: 12, padding: 16, alignItems: 'center', marginTop: 12 }}>
        <Text style={{ color: colors.text }}>I don't have referral code</Text>
      </TouchableOpacity>
    </View>
  );
}
